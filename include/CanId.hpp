...
namespace sockcanpp {
    ...
    struct CanId {
        ...
        CanId(const CanId& orig):
            _identifier(orig._identifier), _isErrorFrame(orig._isErrorFrame), _isRemoteTransmissionRequest(orig._isRemoteTransmissionRequest),
...
#pragma region "Implicit Cast Operators"
        operator int16_t() const {
            return isStandardFrameId() ? (int16_t)_identifier :
...
#pragma region "Assignment Operators"
        CanId operator=(const int32_t val) {
            uint32_t tmpVal = val;
            auto tmp        = (isValidIdentifier(tmpVal) ? CanId(val) :
                                                           throw system_error(error_code(0x5421, generic_category()), "INVALID CAST: ID is extended or invalid!"));
            return tmp;
        }

        CanId operator=(const uint32_t val) {
            uint32_t tmp = val;
            return (isValidIdentifier(tmp) ? CanId(val) :
                                             throw system_error(error_code(0x5421, generic_category()), "INVALID CAST: ID is extended or invalid!"));
        }
